...
std::tuple<uint16_t,uint16_t> REGBLOCK2(1710,6);
  ...
  case 2:
    ...
    if (host.getState() == COM_IDLE) {
      ...
      if (host.getLastError() != ERR_SUCCESS) {
        ...
        au16data = host.getContainer();
        mydata = u16_to_u8(au16data,host.getcontainerCurrSize());  
        do_send(&sendjob);

        Serial.print("sending.."); 
        for(int i = 0; i<host.getcontainerCurrSize();i++){
